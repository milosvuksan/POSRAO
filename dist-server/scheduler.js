...
export const RESOURCE_IDS = ['billiards', 'table-tennis'];
...
const normalizeName = (name) => name.normalize('NFKC').trim().replace(/\s+/g, ' ').toLocaleLowerCase('sr-Latn');
...
export class Scheduler {
    ...
    assertResource(value) {
        if (!RESOURCE_IDS.includes(value)) {
            throw new AppError(404, 'RESOURCE_NOT_FOUND', 'Traženi sto ne postoji.');
        }
    }
    validateInput(name, pin, duration) {
        if (typeof name !== 'string')
            throw new AppError(400, 'INVALID_NAME', 'Unesite ime.');
        ...
        for (const resourceId of RESOURCE_IDS) {
            while (true) {
                const active = this.db
                    ...
                    .get(resourceId);
                if (!active || active.ends_at === null || active.ends_at > now)
                    break;
    ...
    create(resourceValue, input) {
        ...
        return this.transaction(() => {
            ...
            if (existing)
                throw new AppError(409, 'NAME_BUSY', 'Ovo ime već ima aktivnu prijavu ili mesto u redu.');
            const active = this.db
                .prepare("SELECT id FROM entries WHERE resource_id = ? AND state = 'active'")
                .get(resourceId);
            const state = active ? 'queued' : 'active';
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, resourceId, valid.displayName, valid.normalizedName, securedPin.salt, securedPin.hash, tokenHash(token), state, valid.duration, now, startedAt, endsAt);
    ...
    extend(id, token) {
        return this.transaction(() => {
            ...
            const waiting = this.db
                ...
                .get(row.resource_id);
            if (waiting)
                throw new AppError(409, 'QUEUE_EXISTS', 'Produženje nije dostupno dok neko čeka.');
    ...
    finish(id, token) {
        return this.transaction(() => {
            ...
            this.assertControl(row, token);
            if (row.state !== 'active')
                throw new AppError(409, 'NOT_ACTIVE', 'Ova sesija više nije aktivna.');
    ...
    cancel(id, token) {
        return this.transaction(() => {
            ...
            this.assertControl(row, token);
            if (row.state !== 'queued')
                throw new AppError(409, 'NOT_QUEUED', 'Ova prijava više nije u redu.');
    ...
    stateInTransaction(now) {
        const resources = RESOURCE_IDS.map((resourceId) => {
            const active = this.db
                .prepare("SELECT * FROM entries WHERE resource_id = ? AND state = 'active'")
                .get(resourceId);
